...
func testIdentifier(t *testing.T, exp ast.Expression, value string) bool {
	...
	if !ok {
		t.Errorf("exp not *ast.Identifier.%T%s", exp, exp)
		return false
	}
...
func testInfixExpression(t *testing.T, exp ast.Expression, left interface{}, operator string,
	...
	if opExp.Operator != operator {
		t.Errorf("exp.Operation is not %s.got=%s", operator, opExp.Operator)
	}
...
func TestOperatorPrecedenceParsing(t *testing.T) {
	...
	for _, vv := range tests {
		l := lexer.New(vv.input)
		p := New(l)
		program := p.ParseProgram()
		checkParserErrors(t, p)

		actual := program.String()
		if actual != vv.expected {
			t.Errorf("expected=%q, got=%q", vv.expected, actual)
		}
	}
}
...
func TestIfExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("stmt.Expression is not ast.IfExpressoin.got=%T",
			stmt.Expression)
	}
...
func TestFunctionLiteralParsing(t *testing.T) {
	...
	if len(function.Body.Statements) != 1 {
		t.Fatalf("function.Body.statements has not 1 arguments.got=%d\n",
...
func TestCallExpressionParsing(t *testing.T) {
	...
	checkParserErrors(t, p)
	if len(program.Statements) != 1 {
		t.Fatalf("program.Statements does not contain %d arguments.got=%d\n",
			1, len(program.Statements))
	}
	stmt := program.Statements[0].(*ast.ExpressionStatement)
...
func TestMacroLiteralParsint(t *testing.T) {
	input := `macro(x, y) { x + y; }`
	l := lexer.New(input)
	p := New(l)
	program := p.ParseProgram()
